...
class ProducerNotificationService
{
    ...
    private function createSystemNotification(User $producer, Order $order)
    {
        ...
        Notification::create([
            'user_id' => $producer->id,
            'type' => 'new_order',
            'title' => 'Novo Pedido Aguardando Confirmação! ⏰',
            'message' => "Você recebeu um novo pedido de {$order->user->name} que precisa de sua confirmação. Produtos: {$productNames}. Total: R$ " . number_format($totalValue, 2, ',', '.') . ". Retirada: " . $order->pickup_date->format('d/m/Y') . " às {$order->pickup_time}.",
            'data' => json_encode([
                'order_id' => $order->id,
    ...
    private function sendWhatsAppNotification(User $producer, Order $order)
    {
        ...
        $message = "⏰ *NOVO PEDIDO - CONFIRMAÇÃO NECESSÁRIA!*\n\n";
        ...
        $message .= "\n⚠️ *AÇÃO NECESSÁRIA:*\n";
        $message .= "Por favor, acesse o sistema e confirme se pode atender este pedido no horário solicitado.\n\n";
        $message .= "🔗 Acesse: " . url('/sales');
