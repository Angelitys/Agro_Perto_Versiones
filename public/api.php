...
// API completa do AgroPerto Marketplace
...
switch ($path) {
    ...
    case '/products':
        if ($method === 'GET') {
            ...
            $stmt = $pdo->prepare("
                INSERT INTO products (name, description, price, stock, stock_quantity, unit, category_id, user_id, active, available_from, available_until, pickup_location, pickup_instructions, created_at, updated_at) 
            ...
            $stmt->execute([
                ...
                $input['category_id'] ?? 1,
                $input['user_id'] ?? 1,
                $input['available_from'] ?? date('Y-m-d'),
                $input['available_until'] ?? date('Y-m-d', strtotime('+30 days')),
    ...
    case '/features':
        jsonResponse([
            ...
            'database_status' => 'Funcionando com SQLite',
    ...
    default:
        ...
        if (preg_match('/^\/orders\/(\d+)\/confirm-delivery$/', $path, $matches)) {
            ...
            if ($method === 'POST') {
                $stmt = $pdo->prepare("
                    UPDATE orders 
                    SET status = 'delivered', delivery_confirmed_at = datetime('now'), delivery_notes = ? 
                    WHERE id = ?
                ");
